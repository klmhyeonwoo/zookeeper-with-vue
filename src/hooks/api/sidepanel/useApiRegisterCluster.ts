...
export const useApiRegisterCluster = ()  => {
    ...
    const registerCluster = async (address: string, name: string) => {
        const res = await api.post(`/api/zkui/clusters`, {
            host: address,
            name: name
        }, {
            headers: {
    ...
    return useMutation({
        mutationFn: ({address, name}: {address: string, name: string}) => registerCluster(address, name),
        onSuccess: (res: registerResponse) => {
            console.log("useApiRegisterCluster Success:", res);
